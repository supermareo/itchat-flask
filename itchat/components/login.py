...
def login(self, enableCmdQR=False, picDir=None, qrCallback=None, loginCallback=None, exitCallback=None, uid=None):
    ...
    while self.isLogging:
        ...
        if uuid:
            ...
            qrStorage = self.get_QR(enableCmdQR=enableCmdQR,
                                    picDir=picDir, qrCallback=qrCallback, uid=uid)
            logger.info('Please scan the QR code to log in.')
        isLoggedIn = False
        while not isLoggedIn:
            status = self.check_login()
            if hasattr(qrCallback, '__call__'):
                qrCallback(uuid=self.uuid, status=status, qrcode=qrStorage.getvalue(), uid=uid)
            if status == '200':
                isLoggedIn = True
            elif status == '201':
...
def get_QR(self, uuid=None, enableCmdQR=False, picDir=None, qrCallback=None, uid=None):
    ...
    if hasattr(qrCallback, '__call__'):
        qrCallback(uuid=uuid, status='0', qrcode=qrStorage.getvalue(), uid=uid)
    else:
